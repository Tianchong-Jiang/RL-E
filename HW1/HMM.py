...
class HMM(object):
    ...
    def train(self, observations):
        ...
        for iter in range(num_iters):
            ...
            # update params
            self.pi = gammas[:, 0, None] / np.sum(gammas[:, 0, None])
    ...
    def viterbi(self, observation):
        ...
        predictions = np.zeros(observation.shape[0])

        # compute delta and pre
        deltas = np.zeros([self.numStates, observation.shape[0]])
        pres = np.zeros([self.numStates, observation.shape[0]])
    ...
    def computeAlphas(self, observation):
        ...
        alphas = np.zeros([self.numStates, observation.shape[0]])
        log_scales = np.zeros(observation.shape[0])

        # base
        alphas[:, 0] = self.M[observation[0], :] * self.pi[:,0]
    ...
    def computeBetas(self, observation, alphas):
        ...
        betas = np.zeros([self.numStates, observation.shape[0]])
    ...
    def computeXis(self, alphas, betas, observation):
        ...
        # shape: (j, i, t)
        xis = np.zeros([self.numStates, self.numStates, observation.shape[0] - 1])
        for t in range(observation.shape[0] - 1):
            sum = 0
            for i in range(self.numStates):
                for j in range(self.numStates):
                    xis[i, j, t] = alphas[i, t] * self.T[j, i]\
    ...
    def getAllLogOutputProb(self, observation, states):
        ...
        likelihoods = np.zeros(states.shape[0])
        for t in range(states.shape[0]):
            likelihoods[t] = self.getLogOutputProb(observation[t], states[t])
        # pdb.set_trace()
        return np.sum(likelihoods)

    def getAllObsInds(self, observations):
        ...
        observations = np.array(np.flip(observations), dtype=np.int16)
        return observations
    ...
    def sub2ind_all(self, states):
        ...
        states_linear = np.zeros([states.shape[0], states.shape[1]])
